...
namespace std {
    template<>
    struct hash<nndef::E_NN_TYPE> {
        size_t operator()(const nndef::E_NN_TYPE& type) const {
            return hash<underlying_type<nndef::E_NN_TYPE>::type>()(
                static_cast<underlying_type<nndef::E_NN_TYPE>::type>(type)
            );
        }
    };
}